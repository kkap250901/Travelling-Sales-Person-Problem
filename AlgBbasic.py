...
#This function chooses 2 random indexes and switches cities on those 2 indexes
def mutation(cities):
    probability = random.random() 
    if probability < mutation_probability:
        first_index = random.randint(0, num_cities - 1) #First city to be swaped
        second_index = random.randint(0, num_cities - 1) # Second city to be swaped
        cities[first_index], cities[second_index] = cities[second_index], cities[first_index]
    return cities


# One point Crossover Funciton 
def crossover(fitness, local_population):
    ...
    # print(parent1)
    # print(parent2)
    # print(calculateDistance([parent1, parent2]))
    # input()

    # now just adding the positions of both parents
    #Perfoming the swap here
    ...
    # Due to duplicates a funciton written here 
    ...
    # Removing the Duplicates and find the 2 children 
    child1_no_dup = remove_duplicates(child1, child2)
    child2_no_dup = remove_duplicates(child2, child1)#


    #Calculating the child with the lower dis and choosing that
...
def geneticAlgorithm():
    ...
    while time.time() - start_time < 55:
        ...
        #Calculating the fitness of the distances
        fitness = calculate_fitness(distance_of_routes)

        #Creating the new population with crossover and mutations
        New_Pop = []
        ...
        # Making the new pop to the old one 
        local_population = New_Pop

        # Calculating the distance array of the new pop
        distance1 = calculateDistance(local_population)

        # Finding the min dis
        min_dis = min(distance1)

        #Corresponding route to that distance
        route = local_population[distance1.index(min_dis)]

        # Updating the global min distance
        if min_dis <  best_dis: 
            best_dis = min_dis
            best_tour = route
        else:
